...
const snippet = (body: string) => body.replace(/%%(\w+)%%/g, "…").replace(/\*\*/g, "").replace(/\n+/g, " ").slice(0, 150);
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">The wording of your NOCs, mobilization and undertaking letters. Pick a ready-made one, edit it with a live preview, and it&apos;s used the next time you generate a letter.</p>
        </div>
      ...
      {!branchId ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {templates.map((t) => {
            ...
            const original = presetByKey(t.presetKey) && presetByKey(t.presetKey)!.body === t.remarksText;
            return (
              <Link key={t.id} href={`/letter-templates/${t.id}`} className="card block p-4 transition hover:border-[var(--brand-primary)]">
                ...
                <p className="mt-0.5 text-xs text-muted">{t.category ?? "No letter type"}{n > 0 ? ` · used by ${n} NOC${n === 1 ? "" : "s"}` : ""}</p>
                <p className="mt-2 line-clamp-3 text-xs text-secondary">{snippet(t.remarksText)}</p>
